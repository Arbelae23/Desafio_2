...
void Creditos::inicializarArreglos() {
    // Inicializar todas las categorias
...
// METODOS PARA AGREGAR (SIN eliminacion)
...
int Creditos::obtenerCantidadProductores() const {
    return cantidad_productores;
}

int Creditos::obtenerCantidadMusicos() const {
    return cantidad_musicos;
}

int Creditos::obtenerCantidadCompositores() const {
    return cantidad_compositores;
}

int Creditos::obtenerTotalPersonas() const {
    return cantidad_productores + cantidad_musicos + cantidad_compositores;
}
...
void Creditos::mostrarCreditos() const {
    ...
    if (obtenerTotalPersonas() == 0) {
        std::cout << "No hay creditos registrados.\n";
    }

    std::cout << "\nTOTAL: " << obtenerTotalPersonas() << " personas en los creditos\n";
}
